...
class ImageConverter
{
      ...
      std::vector<std::vector<double>> resultCup; // {ellipseRect.center.x, ellipseRect.center.y, color}   color [ 1 for red , 0 for green ]
...
public:
      ...
      bool mission_camera(cup_detect::mission_camera::Request &mis, cup_detect::mission_camera::Response &cam)
      {
            ...
            if (cli_.call(t2_srv))
            {
                  centerX_up = t2_srv.response.robot_x + 100;
                  centerX_down = t2_srv.response.robot_x - 100;
      ...
      void find_Ellipse(std::vector<cv::Point> processContours_i, double color)
      {
            ...
            {
                  std::vector<double> aNewCup = {ellipseRect.center.x, ellipseRect.center.y, color};
      ...
      void printEllipse(cv::Mat img_result)
      {
            ...
            for (int i = 0; i < resultCup.size(); i++)
            {
                  ...
                  if (pow(distance, 2) > (pow(t2_srv.response.robot_x - resultCup[i][0], 2) + pow(t2_srv.response.robot_y - resultCup[i][1], 2)) && needColor == resultCup[i][2] && checkCoor(resultCup[i][0], resultCup[i][1]))
                  {
            ...
            if (abs(centerX - resultRect[min_i].center.x) > 40 && abs(centerY - resultRect[min_i].center.y) > 30)
            {
                  cv::Point2i targetCenter;
                  targetCenter.x = centerX;
                  targetCenter.y = centerY;
                  cv::circle(img_result, targetCenter, 3, cv::Scalar(0, 255, 255), 2);
                  cv::circle(img_result, targetCenter, 7, cv::Scalar(0, 255, 255), 2);
            }
